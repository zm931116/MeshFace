...
def Euclidean_loss(predictions, annotations,Mask = None, alpha = None):
    diff = tf.subtract(predictions,annotations)
    if not Mask is None:
        assert alpha is not None
        diff = tf.multiply(diff,Mask)
        diff = tf.multiply(diff,alpha)
    l2_sum = tf.reduce_sum(tf.square(diff))
    l2_norm = tf.sqrt(l2_sum)
    return l2_norm