...
class VGG_FCN_SMALL:
    ...
    def build(self, rgb, train=False, num_classes=20, random_init_fc8=False,
        ...
        self.score_fr = self._score_layer(self.pool3,'score_fr',num_classes = 512)

        self.upscore2 = self._upscore_layer(self.score_fr,shape = tf.shape(self.pool2),num_classes=128, debug=debug,name = 'upscore2',ksize= 4, stride =2)
        ...
        self.upscore4 = self._upscore_layer(self.fuse_pool2,shape = tf.shape(self.pool1),num_classes =32,debug = debug,name = 'upscore4', ksize = 4,stride = 2)
        self.score_pool1 = self._score_layer(self.pool1, 'score_pool1', num_classes = 32)
        self.fuse_pool1 = tf.add(self.upscore4, self.score_pool1)

        self.upscore8 = self._upscore_layer(self.fuse_pool1,shape = tf.shape(rgb), num_classes= 3, debug = debug, name = 'output_node', ksize = 4, stride = 2)
        self.pred_img = tf.squeeze(self.upscore8)
    ...
    def _score_layer(self, bottom, name, num_classes):
        '''
        1*1 CONVOLUTION 1*1*in_features* num_classes
        :param bottom:
        :param name:
...
#TODO: Substitutes logits using L2 Norm
