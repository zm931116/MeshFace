...
from vgg_fcn import loss, vgg_unet_small
...
def train(train_dir, val_dir, max_epoch, model_path, log_dir, learning_rate,start_epoch = 0,save_interval=2, val_interval=100, display=100,
          from_scrach=True):
    ...
    tf.summary.image('input', input_holder, max_outputs=3)
    tf.summary.image('input_target', target_holder, max_outputs=3)

    vgg_fcn = vgg_unet_small.VGG_UNET_SMALL(vgg16_npy_path='../vgg16.npy')
    with tf.name_scope('vgg_UNet'):
        vgg_fcn.build(input_holder, train=True)

    l2_loss = loss.Euclidean_loss(vgg_fcn.pred_img, target_holder) / batch_size
    optimizer = _configure_optimizer(optimizer_name='adam',learning_rate = learning_rate)
    train_op = optimizer.minimize(l2_loss, global_step)
    tf.summary.image('prediction', vgg_fcn.pred_img, max_outputs=3)
    ...
    try:
        for step in range(start_step, MAX_STEP):
            ...
            if i * batch_size > 27000 * save_interval:
                epoch = epoch + save_interval
                i = 0
                model_name = os.path.join(model_path, 'vgg_UNet18')
